...
export default function CameraFeed() {
  ...
  const analyzeForAccidents = (predictions) => {
      ...
      for (let i = 0; i < vehicles.length; i++) {
        for (let j = i + 1; j < vehicles.length; j++) {
          ...
          if (dist < 120) {
            ...
            const accidentKey = `collision-${Math.floor(Date.now() / 10000)}`
    ...
    if (people.length > 0 && vehicles.length > 0) {
      people.forEach((person) => {
        vehicles.forEach((vehicle) => {
          ...
          if (dist < 100) {
            const accidentKey = `pedestrian-${Math.floor(Date.now() / 10000)}`
            if (!lastAccidentTimeRef.current[accidentKey]) {
              lastAccidentTimeRef.current[accidentKey] = true
  ...
  const reportAccident = async (type, description, severity) => {
    const newAccident = {
  ...
  const analyzeForAccidents = (predictions) => {
      ...
      for (let i = 0; i < vehicles.length; i++) {
        for (let j = i + 1; j < vehicles.length; j++) {
          ...
          if (dist < 100) {
            ...
            if (!lastAccidentTimeRef.current[accidentKey]) {
              lastAccidentTimeRef.current[accidentKey] = true
              reportAccident('Collision', `Potential collision detected between ${vehicles[i].class} and ${vehicles[j].class}`, 4)
            }
          }
        }
      }
    ...
    // Detect anomalies - person near vehicles at high speed
    const people = predictions.filter((p) => p.class === 'person')
    if (people.length > 0 && vehicles.length > 0) {
      people.forEach((person) => {
        vehicles.forEach((vehicle) => {
          ...
          if (dist < 80) {
            ...
            if (!lastAccidentTimeRef.current[accidentKey]) {
              lastAccidentTimeRef.current[accidentKey] = true
              reportAccident('Pedestrian Incident', 'Pedestrian detected near vehicle - possible accident', 5)
            }
          }
        })
      })
  ...
  const reportAccident = async (type, description, severity) => {
    const newAccident = {
      ...
      lat: 37.7749,
      lng: -122.4194,
      location: 'Camera Feed Detection',
    }
  ...
  const drawDetections = (predictions) => {
    ...
    ctx.fillText(`Objects: ${predictions.length}`, 10, 35)

    predictions.forEach((prediction) => {
      const [x, y, width, height] = prediction.bbox
      const isVehicle = ['car', 'truck', 'bus', 'motorcycle'].includes(prediction.class)
      const color = isVehicle ? 'rgba(255, 100, 100, 0.95)' : 'rgba(95, 195, 255, 0.95)'
      const label = `${prediction.class} ${(prediction.score * 100).toFixed(0)}%`
  ...
  return (
    <section className="camera-card">
      ...
      <div className="camera-preview">
        <video ref={videoRef} className="camera-video" autoPlay playsInline muted />
        <canvas ref={canvasRef} className="camera-overlay" />
        {!stream && <div className="camera-placeholder">Start the camera to begin real-time detection.</div>}
      </div>
